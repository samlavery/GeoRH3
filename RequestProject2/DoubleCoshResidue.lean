...
cosh-kernel pair a **classifier** between balanced (β = CoshBalance) and unbalanced
(β ≠ CoshBalance) harmonic configurations. The residue is the
...
* **(S2)** `R_double(β, r) > 0` for β ≠ CoshBalance, r > 0, r ≠ 1 — unbalanced class lifted.
* **(S3)** `R_double(β, r) = R_double(1 − β, r)` — the residue measures |δ|, not sign.
* **(S4)** `|β₁ − 1/2| < |β₂ − 1/2| ⟹ R_double(β₁, r) < R_double(β₂, r)` at r > 1.
* **(S5)** For β ≠ CoshBalance, `1 < r₁ < r₂ ⟹ R_double(β, r₁) < R_double(β, r₂)`.
...
/-- **(S1) Balanced class pinned at zero.** At β = CoshBalance the residue vanishes
...
/-- **(S2) Unbalanced class strictly positive.** For β ≠ CoshBalance at any scale
...
/-- **(S5) Separation widens with scale.** For fixed unbalanced β ≠ CoshBalance, the
residue strictly increases as r grows (for r > 1). -/
theorem residue_mono_scale {β : ℝ} (hβ : β ≠ CoshBalance)
    {r₁ r₂ : ℝ} (hr₁ : 1 < r₁) (hr₁₂ : r₁ < r₂) :
    pairAgreementDefect r₁ β < pairAgreementDefect r₂ β := by
  rw [pairAgreementDefect_sinh_factor, pairAgreementDefect_sinh_factor]
...
theorem classify_at_scale {r : ℝ} (hr : 1 < r) {β : ℝ} (hβ : β ≠ CoshBalance) :
...
theorem widening_gap {β : ℝ} (hβ : β ≠ CoshBalance) {r₁ r₂ : ℝ}
    (hr₁ : 1 < r₁) (hr₁₂ : r₁ < r₂) :
...
theorem classifier_iff_online (ρ : ℂ) (_hρ : ρ ∈ ZD.NontrivialZeros)
    {r : ℝ} (hr : 1 < r) :
    pairAgreementDefect r ρ.re = 0 ↔ ρ.re = CoshBalance :=
