...
theorem zeta_zero_classifier_dichotomy
  ...
  rcases Classical.em (ρ.re = CoshBalance) with hon | hoff
  · left
    intro r; rw [hon, CoshBalance_eq_half]; exact residue_balanced r
  · right
    refine ⟨?_, ?_⟩
    · intro r hr hr1
...
theorem rh_iff_classifier_at_scale {r : ℝ} (hr : 1 < r) :
  ...
  · intro h ρ hρ
    rw [h ρ hρ, CoshBalance_eq_half]; exact residue_balanced r
...
structure ZetaZeroClassifierReport (ρ : ℂ) : Prop where
  ...
  residue_at_pi_third_zero_iff_online :
    pairAgreementDefect (Real.pi / 3) ρ.re = 0 ↔ ρ.re = CoshBalance
...
def classifierReport (ρ : ℂ) (hρ : ρ ∈ ZD.NontrivialZeros) :
  ...
  on_line_residue_zero := fun h r => by
    rw [h, CoshBalance_eq_half]; exact residue_balanced r
