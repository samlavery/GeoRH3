...
/-- Bridge between indexed finite Li sums and `FiniteRH.paired_li_sum` when the
first `K` indexed zero pairs have no duplicate `(σ, γ)` entries. -/
theorem li_partial_sum_eq_paired_li_sum_image_of_inj
    (Z : ZetaZeroData) (K n : ℕ)
    (h_inj : Set.InjOn (fun j : ℕ => (Z.sigma j, Z.gamma j))
      {j : ℕ | j < K}) :
    li_partial_sum Z K n =
      paired_li_sum ((Finset.range K).image fun j => (Z.sigma j, Z.gamma j)) n := by
  unfold li_partial_sum paired_li_sum
  rw [Finset.sum_image]
  · intro a ha b hb hab
    exact h_inj (Finset.mem_range.mp ha) (Finset.mem_range.mp hb) hab

/-- Indexed finite RH: the first `K` indexed zero pairs are on-line if their
indexed Li sum is bounded below and the first `K` `(σ, γ)` pairs are distinct. -/
theorem first_K_on_line_of_pair_injective (Z : ZetaZeroData) (K : ℕ)
    (h_inj : Set.InjOn (fun j : ℕ => (Z.sigma j, Z.gamma j))
      {j : ℕ | j < K})
    (h_bdd : ∃ C : ℝ, ∀ n : ℕ, C ≤ li_partial_sum Z K n) :
    ∀ j, j < K → Z.sigma j = 1/2 := by
  let pairs : Finset (ℝ × ℝ) := (Finset.range K).image fun j => (Z.sigma j, Z.gamma j)
  have h_nontrivial : ∀ z ∈ pairs, z.2 ≠ 0 := by
    intro z hz
    rcases Finset.mem_image.mp hz with ⟨j, _hj, rfl⟩
    exact Z.gamma_ne_zero j
  have h_bdd_pairs : ∃ M : ℝ, ∀ n : ℕ, M ≤ paired_li_sum pairs n := by
    obtain ⟨C, hC⟩ := h_bdd
    refine ⟨C, fun n => ?_⟩
    rw [← li_partial_sum_eq_paired_li_sum_image_of_inj Z K n h_inj]
    exact hC n
  have h_on_pairs := (finite_rh pairs h_nontrivial).mpr h_bdd_pairs
  intro j hj
  exact h_on_pairs (Z.sigma j, Z.gamma j)
...
theorem first_K_on_line (Z : ZetaZeroData) (K : ℕ)
    (h_bdd : ∃ C : ℝ, ∀ n : ℕ, C ≤ li_partial_sum Z K n) :
    ∀ j, j < K → Z.sigma j = 1/2 := by
  have h_nontrivial : ∀ j ∈ Finset.range K, Z.gamma j ≠ 0 := by
    intro j _hj
    exact Z.gamma_ne_zero j
  have h_bdd_indexed :
      ∃ C : ℝ, ∀ n : ℕ,
        C ≤ indexed_paired_li_sum (Finset.range K) Z.sigma Z.gamma n := by
    obtain ⟨C, hC⟩ := h_bdd
    refine ⟨C, fun n => ?_⟩
    simpa [indexed_paired_li_sum, li_partial_sum] using hC n
  have h_on :=
    (finite_rh_indexed (Finset.range K) Z.sigma Z.gamma h_nontrivial).mpr h_bdd_indexed
  intro j hj
  exact h_on j (Finset.mem_range.mpr hj)
