...
`riemannZeta`, asserting `s.re = CoshBalance` under two exclusions:
...
* `no_offline_zeros_implies_rh` — the core bridge from
  `∀ ρ ∈ NontrivialZeros, ρ.re = CoshBalance` to Mathlib's `RiemannHypothesis`.
...
theorem no_offline_zeros_implies_rh
  ...
  push_neg at h1
  by_cases h0 : 0 < s.re
  · rw [← CoshBalance_eq_half]; exact hline s ⟨h0, h1, hs⟩
  push_neg at h0
  exfalso
  have hne0 : s ≠ 0 := by
