...
This is a theorem about the objects' *origination*, not about GRH (see the closing note). -/
...
/-- **The helix winding is a closed term.** It exists with no function/`L` parameter at all: its type
    ...
    object — `f` contributes nothing. Where the zeros admit **no** `f`-free form
...
/-- **Origination by construction, as one theorem.** The helix has an `L`-free *geometric* builder
    ...
    an analytic `L : ℂ → ℂ`, genuinely vary with it, and admit no `L`-free predicate at all. A
...
/-! ## What this settles — and, honestly, what it does not

`independent_origination_verdict` decides the **ontology**: the zeros are dependently originated on
`L`, the helix is independently originated. That is the asymmetry, proved.

It does **not** prove GRH, and pretending it did would be the costume. Dependent origination says the
zeros are `L`'s property to fix — it does **not**, by itself, fix them on the line. For the dependent
zeros to *inherit* the independent helix's on-line structure you need the bridge

  `the zeros of L  =  the singularities the helix produces`   (`SourceComplete`),

and `HelixSource.sourceComplete_iff_grh` proves that bridge is `↔ GRH` — GRH-equivalent, still open.
So: origination is decided here; on-line-ness is the one remaining lemma, and it is exactly that
bridge, not a consequence of the ontology alone. -/
