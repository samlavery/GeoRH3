...
-/
theorem moebius_helix_norm_sq (sigma gamma : ℝ) (h : sigma ^ 2 + gamma ^ 2 ≠ 0) :
...
theorem moebius_unit_iff (sigma gamma : ℝ) (hg : gamma ≠ 0) :
  ...
  rw [ ← sq_eq_sq₀, moebius_helix_norm_sq ] <;> norm_num [ hg ];
...
lemma circle_recurrence (u : ℂ) (hu : ‖u‖ = 1) {ε : ℝ} (hε : 0 < ε) :
    ∃ m : ℕ, 1 ≤ m ∧ ‖u ^ m - 1‖ < ε := by
...
lemma recur_cofinal (u : ℂ) (hu : ‖u‖ = 1) {ε : ℝ} (hε : 0 < ε) (N : ℕ) :
    ∃ n : ℕ, N ≤ n ∧ ‖u ^ n - 1‖ < ε := by
