...
/-- A midpoint event in the faithful cone pipeline gives the HP spectral-unit readout. -/
theorem pipeline_midpoint_gives_spectral_unit (ρ : ℂ) (hρ : ρ ≠ 0)
    (hmid : pipeline ρ.re = 1 / 2) :
    Complex.normSq (SpectralSide.w ρ) = 1 := by
  have hre : ρ.re = 1 / 2 := (pipeline_midpoint_iff ρ.re).mp hmid
  exact (SpectralSide.w_unit_iff_half ρ hρ).mpr hre

/-- A midpoint event in the faithful cone pipeline gives the Möbius/Pythagorean readout. -/
theorem pipeline_midpoint_gives_moebius_pythagoras (ρ : ℂ) (hγ : ρ.im ≠ 0)
    (hmid : pipeline ρ.re = 1 / 2) :
    (moebius_helix ρ.re ρ.im).re ^ 2 + (moebius_helix ρ.re ρ.im).im ^ 2 = 1 := by
  have hre : ρ.re = 1 / 2 := (pipeline_midpoint_iff ρ.re).mp hmid
  exact (readout_pythagoras_iff_online ρ.re ρ.im hγ).mpr hre

/-- The HP projected event represented by the faithful cone pipeline: a source event together with
the retained midpoint equation. -/
def HPProjectedEvent {N : ℕ} [NeZero N] (χ : DirichletCharacter ℂ N) (ρ : ℂ) : Prop :=
  Nonempty (SourceFiberEvent χ ρ) ∧ pipeline ρ.re = 1 / 2

/-- Lower dimensions create no event absent from the source, for the cone-pipeline event. -/
noncomputable def HPProjectedEvent.lower_dimensions_create_no_events {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ} :
    HPProjectedEvent χ ρ → SourceFiberEvent χ ρ :=
  fun h => Classical.choice h.1

/-- Source-to-projection transport once the source crossing supplies the retained midpoint event. -/
theorem HPProjectedEvent.source_event_faithfully_projected_of_midpoint {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ}
    (hsrc : SourceFiberEvent χ ρ) (hmid : pipeline ρ.re = 1 / 2) :
    HPProjectedEvent χ ρ :=
  ⟨⟨hsrc⟩, hmid⟩

/-- The cone-pipeline projected event supplies the exact Pythagorean atom consumed by
`FaithfulDimensionalProjection.projected_readout_pythagorean`. -/
theorem HPProjectedEvent.projected_readout_pythagorean {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ}
    (_hEuler : EulerProductDiscriminator χ) (_hcap : FiberCaptureEvent χ ρ)
    (hsrc : SourceFiberEvent χ ρ) (_hNoDrift : hsrc.driftRate.re = 0)
    (hproj : HPProjectedEvent χ ρ) :
    Complex.normSq (SpectralSide.w ρ) = 1 :=
  pipeline_midpoint_gives_spectral_unit ρ hsrc.zero_ne_zero hproj.2

/-- The geometric midpoint proof in `π/3` coordinates.

If a source crossing is registered at the helix midpoint `π/6`, then the `π/3` chart gives
`ρ.re = 1/2`; the faithful log pipeline is the identity on the retained coordinate, so the projected
readout is also `1/2`. -/
theorem sourceFiber_pipeline_midpoint_of_source_coordinate_midpoint {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ}
    (hsrc : SourceFiberEvent χ ρ) (hcoord : hsrc.source_coordinate = Real.pi / 6) :
    pipeline ρ.re = 1 / 2 := by
  have hchart : arcChart ρ.re = Real.pi / 6 := by
    rw [← hsrc.piThird_coordinate, hcoord]
  have hre : ρ.re = 1 / 2 := (arcChart_line ρ.re).mp hchart
  rw [pipeline_id, hre]
...
/-- A source sign flip forces the retained source coordinate to be the `π/3` midpoint. -/
theorem sourceFiber_source_coordinate_midpoint_of_signFlip {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ} (hsrc : SourceFiberEvent χ ρ)
    (hflip : piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    hsrc.source_coordinate = Real.pi / 6 :=
  (piThirdSignFlip_fixed_iff_midpoint hsrc.source_coordinate).mp hflip

/-- A source sign flip supplies the retained midpoint event in the faithful cone pipeline. -/
theorem sourceFiber_pipeline_midpoint_of_signFlip {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ} (hsrc : SourceFiberEvent χ ρ)
    (hflip : piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    pipeline ρ.re = 1 / 2 :=
  sourceFiber_pipeline_midpoint_of_source_coordinate_midpoint hsrc
    (sourceFiber_source_coordinate_midpoint_of_signFlip hsrc hflip)

/-- A source event together with the source-side sign flip at the `π/3` crossing.  The flip is
the geometric atom: by `piThirdSignFlip_fixed_iff_midpoint`, it forces the source coordinate to be
the midpoint `π/6`, which the `arcChart` field then transports to the retained `1/2` readout. -/
structure SourceCrossingFiberEvent {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N) (ρ : ℂ) where
  source : SourceFiberEvent χ ρ
  source_sign_flip : piThirdSignFlip source.source_coordinate = source.source_coordinate

/-- A source crossing event supplies the retained midpoint event in the faithful cone pipeline. -/
theorem SourceCrossingFiberEvent.pipeline_midpoint {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ} (hcross : SourceCrossingFiberEvent χ ρ) :
    pipeline ρ.re = 1 / 2 :=
  sourceFiber_pipeline_midpoint_of_signFlip hcross.source hcross.source_sign_flip

/-- Source-crossing completeness for one Dirichlet channel: every captured fiber event is realized
on the 3D source as a sign-changing crossing. -/
def SourceCrossingComplete3D {N : ℕ} [NeZero N] (χ : DirichletCharacter ℂ N) : Prop :=
  ∀ ρ : ℂ, FiberCaptureEvent χ ρ → Nonempty (SourceCrossingFiberEvent χ ρ)

/-- Source completeness plus the source sign-flip law builds source-crossing completeness. -/
noncomputable def sourceCrossingComplete3DOfSourceSignFlip {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N) (hχ : χ ≠ 1)
    (hflip : ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
      piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    SourceCrossingComplete3D χ := by
  intro ρ hcap
  rcases sourceComplete3D_of_HP χ hχ ρ hcap with ⟨hsrc⟩
  exact ⟨{ source := hsrc, source_sign_flip := hflip ρ hsrc }⟩

/-- Build the exact per-character HP faithfulness structure from source-crossing completeness. -/
noncomputable def faithfulDimensionalProjectionOfSourceCrossingComplete {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N) (hcross : SourceCrossingComplete3D χ) :
    FaithfulDimensionalProjection χ where
  projected_event := HPProjectedEvent χ
  source_event_faithfully_projected := by
    intro ρ hcap hsrc
    rcases hcross ρ hcap with ⟨hcrossρ⟩
    exact HPProjectedEvent.source_event_faithfully_projected_of_midpoint hsrc
      hcrossρ.pipeline_midpoint
  lower_dimensions_create_no_events := by
    intro ρ hproj
    exact HPProjectedEvent.lower_dimensions_create_no_events hproj
  projected_readout_pythagorean := by
    intro ρ hEuler hcap hsrc hNoDrift hproj
    exact HPProjectedEvent.projected_readout_pythagorean hEuler hcap hsrc hNoDrift hproj

/-- Per-character closure once every captured source event is realized as a sign-changing crossing. -/
theorem GRH_of_HP_sourceCrossingComplete {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N) (hχ : χ ≠ 1) (hcross : SourceCrossingComplete3D χ) :
    GRHSpectral.GRH χ :=
  GRH_of_HP_and_Faithfulness χ hχ
    (faithfulDimensionalProjectionOfSourceCrossingComplete χ hcross)

/-- Source-crossing completeness for every non-principal Dirichlet channel. -/
structure SourceCrossingComplete3DAll where
  character :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      SourceCrossingComplete3D χ

/-- All-channel source-crossing completeness from the all-channel source sign-flip law. -/
noncomputable def sourceCrossingComplete3DAllOfSourceSignFlip
    (hflip : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
        piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    SourceCrossingComplete3DAll where
  character := by
    intro M hM χ hχ
    exact sourceCrossingComplete3DOfSourceSignFlip χ hχ (hflip M χ hχ)

/-- Build all-channel HP faithfulness from all-channel source-crossing completeness. -/
noncomputable def faithfulDimensionalProjectionAllOfSourceCrossingComplete
    (hcross : SourceCrossingComplete3DAll) :
    FaithfulDimensionalProjectionAll where
  character := by
    intro M hM χ hχ
    exact faithfulDimensionalProjectionOfSourceCrossingComplete χ
      (hcross.character M χ hχ)

/-- All non-principal closures from the HP chain plus source-crossing completeness. -/
theorem GRHComplete_of_HP_sourceCrossingComplete (hcross : SourceCrossingComplete3DAll) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 → GRHSpectral.GRH χ :=
  GRHComplete_of_HP_and_Faithfulness
    (faithfulDimensionalProjectionAllOfSourceCrossingComplete hcross)

/-- All non-principal closures via the first-class source-crossing event, with crossings built
from the source sign-flip law. -/
theorem GRHComplete_of_HP_sourceCrossingComplete_sourceSignFlip
    (hflip : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
        piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 → GRHSpectral.GRH χ :=
  GRHComplete_of_HP_sourceCrossingComplete
    (sourceCrossingComplete3DAllOfSourceSignFlip hflip)

/-- Build the exact per-character HP faithfulness structure from source-to-midpoint transport in
the faithful cone pipeline. -/
noncomputable def faithfulDimensionalProjectionOfMidpointTransport {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N)
    (hmid : ∀ ρ : ℂ, SourceFiberEvent χ ρ → pipeline ρ.re = 1 / 2) :
    FaithfulDimensionalProjection χ where
  projected_event := HPProjectedEvent χ
  source_event_faithfully_projected := by
    intro ρ _hcap hsrc
    exact HPProjectedEvent.source_event_faithfully_projected_of_midpoint hsrc (hmid ρ hsrc)
  lower_dimensions_create_no_events := by
    intro ρ hproj
    exact HPProjectedEvent.lower_dimensions_create_no_events hproj
  projected_readout_pythagorean := by
...
/-- Build the all-channel HP faithfulness structure from source-to-midpoint transport in every
non-principal channel. -/
noncomputable def faithfulDimensionalProjectionAllOfMidpointTransport
    (hmid : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, SourceFiberEvent χ ρ → pipeline ρ.re = 1 / 2) :
    FaithfulDimensionalProjectionAll where
  character := by
    intro M hM χ hχ
    exact faithfulDimensionalProjectionOfMidpointTransport χ (hmid M χ hχ)

/-- All non-principal closures follow once each source crossing is transported to the retained
midpoint event in the faithful cone pipeline. -/
theorem GRHComplete_of_HP_midpointTransport
    (hmid : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, SourceFiberEvent χ ρ → pipeline ρ.re = 1 / 2) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 → GRHSpectral.GRH χ :=
  GRHComplete_of_HP_and_Faithfulness
    (faithfulDimensionalProjectionAllOfMidpointTransport hmid)

/-- Build all-channel HP faithfulness from the geometric source-coordinate midpoint law. -/
noncomputable def faithfulDimensionalProjectionAllOfSourceCoordinateMidpoint
    (hcoord : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) → hsrc.source_coordinate = Real.pi / 6) :
    FaithfulDimensionalProjectionAll :=
  faithfulDimensionalProjectionAllOfMidpointTransport (by
    intro M hM χ hχ ρ hsrc
    exact sourceFiber_pipeline_midpoint_of_source_coordinate_midpoint hsrc
      (hcoord M χ hχ ρ hsrc))

/-- Non-principal closure from the geometric midpoint projection at each source crossing. -/
theorem GRHComplete_of_HP_sourceCoordinateMidpoint
    (hcoord : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) → hsrc.source_coordinate = Real.pi / 6) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 → GRHSpectral.GRH χ :=
  GRHComplete_of_HP_and_Faithfulness
    (faithfulDimensionalProjectionAllOfSourceCoordinateMidpoint hcoord)

/-- Build all-channel HP faithfulness from the geometric source sign-flip law. -/
noncomputable def faithfulDimensionalProjectionAllOfSourceSignFlip
    (hflip : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
        piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    FaithfulDimensionalProjectionAll :=
  faithfulDimensionalProjectionAllOfMidpointTransport (by
    intro M hM χ hχ ρ hsrc
    exact sourceFiber_pipeline_midpoint_of_signFlip hsrc (hflip M χ hχ ρ hsrc))

/-- Non-principal closure from source sign flips at each crossing. -/
theorem GRHComplete_of_HP_sourceSignFlip
    (hflip : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
        piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 → GRHSpectral.GRH χ :=
  GRHComplete_of_HP_and_Faithfulness
    (faithfulDimensionalProjectionAllOfSourceSignFlip hflip)

/-- Source sign flip at the `π/3` crossing forces zero radial loss in the completed pole
embedding.  This is the local geometry lemma used to build the no-radial-drift source chain:
sign flip fixes the midpoint, the source event translates that midpoint through `arcChart`, and
`zero_embed` stores radial loss as `ρ.re - 1/2`. -/
theorem sourceFiber_zero_embed_radial_zero_of_signFlip {N : ℕ} [NeZero N]
    {χ : DirichletCharacter ℂ N} {ρ : ℂ} (hsrc : SourceFiberEvent χ ρ)
    (hflip : piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) (x : ℝ) :
    (zero_embed ρ.re ρ.im x).radial = 0 := by
  have hmid : pipeline ρ.re = 1 / 2 :=
    sourceFiber_pipeline_midpoint_of_signFlip hsrc hflip
  have hre : ρ.re = 1 / 2 := (pipeline_midpoint_iff ρ.re).mp hmid
  exact (radial_loss_zero_iff ρ.re ρ.im x).mpr hre

/-- A source sign flip at every captured crossing builds the completed-pole no-radial-drift
source chain.  The only calculation is the coordinate one: sign flip fixes the `π/3` midpoint,
the source event identifies that midpoint with `arcChart ρ.re`, and `zero_embed` records radial
loss as `ρ.re - 1/2`. -/
noncomputable def poleHelixNoRadialDriftChainOfSourceSignFlip {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N) (hχ : χ ≠ 1)
    (hflip : ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
      piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
  ...
  source_no_drift := by
    ...
    exact sourceFiber_zero_embed_radial_zero_of_signFlip hsrc (hflip ρ hsrc) x

/-- All-channel no-radial-drift source chains from the all-channel source sign-flip law. -/
noncomputable def poleHelixNoRadialDriftChainAllOfSourceSignFlip
    (hflip : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
        piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      HelixReadsGRH.PoleHelixNoRadialDriftChain χ := by
  intro M hM χ hχ
  exact poleHelixNoRadialDriftChainOfSourceSignFlip χ hχ (hflip M χ hχ)

/-- Build the exact per-character HP faithfulness structure from a completed-pole no-radial-drift
source chain.  This removes the abstract projection atom: for a captured zero, the chain gives
unitary Möbius readout on the completed log-derivative pole, and the cone pipeline records the same
midpoint. -/
noncomputable def faithfulDimensionalProjectionOfNoRadialDriftChain {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N)
  ...
  source_event_faithfully_projected := by
    ...
    have hunit : Complex.normSq (SpectralSide.w ρ) = 1 :=
      HelixReadsGRH.unitary_on_logderiv_poles_of_no_radial_drift_chain χ D ρ hcap
    have hre : ρ.re = 1 / 2 :=
      (SpectralSide.w_unit_iff_half ρ hsrc.zero_ne_zero).mp hunit
    exact HPProjectedEvent.source_event_faithfully_projected_of_midpoint hsrc (by
      rw [pipeline_id, hre])
  lower_dimensions_create_no_events := by
    intro ρ hproj
    exact HPProjectedEvent.lower_dimensions_create_no_events hproj
...
/-- Per-character HP closure from a completed-pole no-radial-drift source chain. -/
theorem GRH_of_HP_noRadialDriftChain {N : ℕ} [NeZero N]
    (χ : DirichletCharacter ℂ N) (hχ : χ ≠ 1)
    (D : HelixReadsGRH.PoleHelixNoRadialDriftChain χ) : GRHSpectral.GRH χ :=
  GRH_of_HP_and_Faithfulness χ hχ
    (faithfulDimensionalProjectionOfNoRadialDriftChain χ D)

/-- Build all-channel HP faithfulness from completed-pole no-radial-drift source chains. -/
noncomputable def faithfulDimensionalProjectionAllOfNoRadialDriftChain
    (D : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      HelixReadsGRH.PoleHelixNoRadialDriftChain χ) :
    FaithfulDimensionalProjectionAll where
  character := by
    intro M hM χ hχ
    exact faithfulDimensionalProjectionOfNoRadialDriftChain χ (D M χ hχ)

/-- Non-principal closure from completed-pole no-radial-drift source chains. -/
theorem GRHComplete_of_HP_noRadialDriftChain
    (D : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      HelixReadsGRH.PoleHelixNoRadialDriftChain χ) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 → GRHSpectral.GRH χ :=
  GRHComplete_of_HP_and_Faithfulness
    (faithfulDimensionalProjectionAllOfNoRadialDriftChain D)

/-- Non-principal closure through the no-radial-drift source-chain route, with that chain built
from the source sign-flip law. -/
theorem GRHComplete_of_HP_noRadialDriftChain_sourceSignFlip
    (hflip : ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 →
      ∀ ρ : ℂ, (hsrc : SourceFiberEvent χ ρ) →
        piThirdSignFlip hsrc.source_coordinate = hsrc.source_coordinate) :
    ∀ (M : ℕ) [NeZero M] (χ : DirichletCharacter ℂ M), χ ≠ 1 → GRHSpectral.GRH χ :=
  GRHComplete_of_HP_noRadialDriftChain
    (poleHelixNoRadialDriftChainAllOfSourceSignFlip hflip)

/-- The zeta/L1 projected event represented by the faithful cone pipeline. -/
def ZetaHPProjectedEvent (ρ : ℂ) : Prop :=
  Nonempty (ZetaSourceFiberEvent ρ) ∧ pipeline ρ.re = 1 / 2

/-- Lower dimensions create no zeta/L1 event absent from the source, for the cone-pipeline event. -/
noncomputable def ZetaHPProjectedEvent.lower_dimensions_create_no_events {ρ : ℂ} :
    ZetaHPProjectedEvent ρ → ZetaSourceFiberEvent ρ :=
  fun h => Classical.choice h.1

/-- Build the exact zeta/L1 faithfulness structure from source construction plus
source-to-midpoint transport in the faithful cone pipeline. -/
noncomputable def faithfulZetaDimensionalProjectionOfMidpointTransport
    (hsrc : ∀ ρ : ℂ, ρ ∈ ZD.NontrivialZeros → ZetaSourceFiberEvent ρ)
    (hmid : ∀ ρ : ℂ, (h : ρ ∈ ZD.NontrivialZeros) →
      pipeline ρ.re = 1 / 2) :
    FaithfulZetaDimensionalProjection where
  projected_event := ZetaHPProjectedEvent
  fiber_capture_faithfully_projected := by
    intro ρ hρ
    exact ⟨⟨hsrc ρ hρ⟩, hmid ρ hρ⟩
  lower_dimensions_create_no_events := by
    intro ρ hproj
    exact ZetaHPProjectedEvent.lower_dimensions_create_no_events hproj
  projected_readout_pythagorean := by
    intro ρ _hEuler _hcap hsrcρ _hNoDrift hproj
    exact pipeline_midpoint_gives_moebius_pythagoras ρ hsrcρ.height_ne_zero hproj.2

/-- The zeta/L1 closure follows once zeta source events are built and each is transported to the
retained midpoint event in the faithful cone pipeline. -/
theorem RH_of_HP_midpointTransport
    (hsrc : ∀ ρ : ℂ, ρ ∈ ZD.NontrivialZeros → ZetaSourceFiberEvent ρ)
    (hmid : ∀ ρ : ℂ, (h : ρ ∈ ZD.NontrivialZeros) →
      pipeline ρ.re = 1 / 2) :
    RiemannHypothesis :=
  RH_of_HP_and_Faithfulness
    (faithfulZetaDimensionalProjectionOfMidpointTransport hsrc hmid)
