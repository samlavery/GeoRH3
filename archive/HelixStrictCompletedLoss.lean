...
/-- **GRH(χ₃) reduces to exactly one atom: the actual zeros' radial loss modes converge.**

    The functional-equation pairing of the χ₃ zeros (`chi3_completedLogDerivPole_one_sub`) is already
    a theorem — so it is folded in here, *not* assumed. What remains is the single honest obligation:
    for every nontrivial zero `ρ`, the radial loss mode `exp((Re ρ − ½)·θ)` tends to a finite limit as
    the helix winds (`θ → ∞`). The FE pairing then reflects `ρ ↦ 1 − ρ`, so finite-limit on both sides
    forces `Re ρ = ½`. This is the "the helix is its own continuation — it does not diverge" statement
    in its exact form: **non-divergence of the completing radial mode is the whole remaining gap.** -/
theorem GRH_chi3_of_radial_modes_converge
    (hconv : ∀ ρ : ℂ, HelixReadsGRH.CompletedLogDerivPole ChiThree.χ3 ρ →
        ∃ L : ℝ, Filter.Tendsto (fun θ : ℝ => Real.exp ((ρ.re - 1 / 2) * θ))
          Filter.atTop (nhds L)) :
    GRHSpectral.GRH ChiThree.χ3 :=
  fun ρ hρ =>
    FEClosure.fe_tends_towards_closure ChiThree.χ3 chi3_completedLogDerivPole_one_sub hconv ρ hρ
...
#print axioms HelixStrictCompletion.GRH_chi3_of_radial_modes_converge
