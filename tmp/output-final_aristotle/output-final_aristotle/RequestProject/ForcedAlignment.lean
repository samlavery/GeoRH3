...
the *frequencies of the loss field's spectrum* — the explicit formula
`loss = −Σ_ρ x^ρ/ρ` means the `γ_ρ` appear as Fourier content. They are a
**readout** of the field, not features on the helix. The helix generates a field;
ζ has zeros; the bridge is the explicit formula.

Helix-intrinsically, RH is not about zeros at all — it is the statement that
the projection loss field is bounded: `ψ(x) − x = O(x^{1/2+ε})`, growth
exponent exactly ½, cylinder not cone.

## Part I — The Möbius reciprocal forces σ = 1/2

The critical line `Re(s) = 1/2` is **derived**, not assumed, via the
mirror-pair/AM-GM mechanism:

1. **The FE involution as reciprocal**: `σ ↦ 1−σ` pairs `ρ` with `1−ρ`.
   Under the Möbius map `w = 1−1/ρ`, this becomes `w ↦ 1/w` — the FE
   converts left-right symmetry into inside/outside asymmetry on the
   unit disk.

2. **The Möbius reciprocal** (proved here): `w(ρ) · w(1−ρ) = 1`, hence
   `‖w(ρ)‖ · ‖w(1−ρ)‖ = 1`. If one partner has norm `r`, the other
   has norm `1/r`.

3. **The mirror-pair defect (AM-GM)**: The combined radial defect of a
   reciprocal pair `{r, 1/r}` is `(1−r) + (1−1/r) = −(r−1)²/r ≤ 0`,
   strictly negative when `r ≠ 1`. This is `r + 1/r ≥ 2` (AM-GM),
   with equality iff `r = 1` (the critical line). Every off-line pair
   drags `λ_n → −∞`.

4. **Unit circle dynamics**: On the line, `‖w‖ = 1` gives pure rotation
   — `‖wⁿ‖ = 1`, bounded forever. Off the line, one spiral contracts
   (`wⁿ → 0`) and the other expands (`wⁿ → ∞`). The expanding side
   grows faster than the contracting side shrinks (AM-GM). The bounded/
   unitary/self-adjoint structure is destroyed. `w ↦ 1/w` fixes only
   the unit circle — the unique escape.
...
See also `TwoFieldSpectral.lean` for the full AM-GM / mirror-pair
argument with all the defect identities and divergence bounds.

## Part II — Two-projection cascade with embedded loss

The two projections factor each zero into its real and imaginary parts:

- **Stage 1 (3D→2D, radial)**: drops `r = n^a`; the radial exponent
  `a = β − ½` carries `Re(ρ) − ½`. Loss field 1 vanishes iff RH.
- **Stage 2 (2D→1D, angular)**: drops the angle; the residual `ψ(x)−x`
  has spectrum `γ = Im(ρ)` — always nonzero.

The two projections supply the resolvent (Green², radial-then-angular).
The alignment constraint is `a = θ* = ½`: the radial scale (field 1)
must match the fluctuation's natural growth (field 2).

## Part III — Two channels (mod-6 characters)
...
Two channels in 3D merge to one object in 2D. The pairing gives the
quadratic form (`|L₀|²+|L₁|²`, manifestly nonneg) that a single field
cannot produce. `ζ_K` has nonneg coefficients (counts ideals).

Pipeline: 2 channels (arithmetic) × 2 stages (geometric).
Stage 1 drops the radial AND merges channels. The two channels supply
the operator; the two stages supply its resolvent.

## Part IV — The Euler engine as positivity source

The Euler product `ζ(s) = ∏_p (1 − p^{−s})^{−1}` is the sieve:
- Composites → 0: `Λ(n) = 0` for non-prime-powers.
- Primes survive: `Λ(p) = log p > 0`.
- Coverage → 1 as more primes included (`Σ 1/p` diverges).
- Feeds the Weil diagonal form `W(f) = Σ f(n)² Λ(n) ≥ 0`.

The combined chain: Euler engine → embedded loss positivity →
Möbius reciprocal → AM-GM defect → forced σ = 1/2.
-/
...
theorem paired_li_unbounded_off_line (sigma gamma : ℝ)
  ...
  · -- By li_helix_un �bounded�_off_line, there exists n such that (li_helix_term sigma gamma n).re < M - 2.
    obtain ⟨n, hn⟩ : ∃ n : ℕ, (li_helix_term sigma gamma n).re < M - 2 := by
      exact li_helix_unbounded_off_line sigma gamma hg h_case1 (M - 2);
    use n;
...
theorem forced_half_from_bounded_li (sigma gamma : ℝ) (hg : gamma ≠ 0)
    ...
    sigma = 1 / 2 := by
  contrapose! hbdd;
  exact paired_li_unbounded_off_line sigma gamma hbdd hg
...
theorem critical_line_iff_bounded_li (sigma gamma : ℝ) (hg : gamma ≠ 0) :
  ...
  · exact forced_half_from_bounded_li sigma gamma hg h
