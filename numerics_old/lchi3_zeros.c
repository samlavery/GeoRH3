...
static void Lchi3_gen(mpc_t res,const mpc_t s){long N=(long)(1.4*fabs(mpfr_get_d(mpc_imagref(s),MPFR_RNDN)))+80,M=32;
...
static void Lline(mpc_t res,const mpfr_t g){
  long N=(long)(1.4*fabs(mpfr_get_d(g,MPFR_RNDN)))+80,M=32; if(N>NMAX)N=NMAX;
...
static double Lam(double gd){mpfr_t g;mpfr_init2(g,PREC);mpfr_set_d(g,gd,MPFR_RNDN);mpc_t L,s,half,gam,bx;mpc_init2(L,PREC);mpc_init2(s,PREC);mpc_init2(half,PREC);mpc_init2(gam,PREC);mpc_init2(bx,PREC);
  Lline(L,g);mpfr_set_d(mpc_realref(s),0.5,MPFR_RNDN);mpfr_set(mpc_imagref(s),g,MPFR_RNDN);mpc_add_ui(half,s,1,MPC_RNDNN);mpc_div_ui(half,half,2,MPC_RNDNN);
  clgamma(gam,half);mpc_exp(gam,gam,MPC_RNDNN);mpfr_t b,pi;mpfr_init2(b,PREC);mpfr_init2(pi,PREC);mpfr_const_pi(pi,MPFR_RNDN);mpfr_set_ui(b,3,MPFR_RNDN);mpfr_div(b,b,pi,MPFR_RNDN);
  cpow_rb(bx,b,half);mpc_mul(bx,bx,gam,MPC_RNDNN);mpc_mul(bx,bx,L,MPC_RNDNN);double v=mpfr_get_d(mpc_realref(bx),MPFR_RNDN);
  mpfr_clear(g);mpc_clear(L);mpc_clear(s);mpc_clear(half);mpc_clear(gam);mpc_clear(bx);mpfr_clear(b);mpfr_clear(pi);return v;}
static void polish(mpfr_t g){mpfr_t h,gp;mpfr_init2(h,PREC);mpfr_init2(gp,PREC);mpfr_set_str(h,"1e-25",10,MPFR_RNDN);
...
int main(int argc,char**argv){mpfr_set_default_prec(PREC);long K=argc>1?atol(argv[1]):100;int doscan=argc>2?atoi(argv[2]):1;
  NMAX=(long)(1.4*(2.0*K+40))+200;LG13=malloc(NMAX*sizeof(mpfr_t));AM13=malloc(NMAX*sizeof(mpfr_t));LG23=malloc(NMAX*sizeof(mpfr_t));AM23=malloc(NMAX*sizeof(mpfr_t));
  ...
  double step=0.04,g=1.0,prev=Lam(g);long found=0;mpfr_t gz,ab;mpfr_init2(gz,PREC);mpfr_init2(ab,PREC);double worst=0;
  while(found<K&&g<200000){double g2=g+step,cur=Lam(g2);if((prev<0)!=(cur<0)){double aa=g,bb=g2,fa=prev;
      ...
      if(found<=20||found%50==0)mpfr_printf("%5ld  %.46Rf   |L|=%.2Re\n",found,gz,ab);}
    prev=cur;g=g2;}
